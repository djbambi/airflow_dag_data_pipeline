...
SAMPLE_DATE_RANGE_DATA = {
    date(2026, 2, 14): {
        "temperature": {"morning": 4.0, "afternoon": 8.0, "evening": 6.0, "night": 2.0}
    },
    date(2026, 2, 15): {
        "temperature": {"morning": 5.0, "afternoon": 9.0, "evening": 6.0, "night": 3.0}
    },
    date(2026, 2, 16): {
        "temperature": {"morning": 3.0, "afternoon": 7.0, "evening": 5.0, "night": 1.0}
    },
}
...
def test_get_mean_daily_temperature_parametrized(
    transformer, start_date, end_date, expected
):
...
def test_get_mean_daily_temperature_raises_for_inverted_date_range(transformer):
    with pytest.raises(ValueError):
        transformer.get_mean_daily_temperature(
            SAMPLE_DATE_RANGE_DATA, date(2026, 2, 16), date(2026, 2, 14)
        )


def test_filter_data_excludes_dates_outside_range(transformer):
...
def test_get_mean_daily_temperature_raises_value_error_for_empty_range(transformer):
    """Test that a ValueError is raised when no data exists in the date range."""
    with pytest.raises(ValueError):
        transformer.get_mean_daily_temperature(
            SAMPLE_DATE_RANGE_DATA, date(2025, 3, 1), date(2025, 3, 7)
        )
